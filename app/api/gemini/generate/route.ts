...
export async function POST(request: Request) {
  console.log("[Gemini Generate App Router root] Received POST request");
  ...
  try {
    ...
    if (!rawType || !topicName || !subtopicName) {
      return new Response(JSON.stringify({ success: false, error: "Missing required parameters (type, topicName, subtopicName)" }), {
        status: 400,
        headers: { "Content-Type": "application/json" }
      });
    }
...
Avoid dry or generic summaries. Craft deep senior-level insight with complete code examples if applicable.`;
      ...
      return new Response(JSON.stringify({ success: false, error: "Invalid type specified" }), {
        status: 400,
        headers: { "Content-Type": "application/json" }
      });
    }
    ...
    console.error("Gemini Generation App Router API Error:", error);
